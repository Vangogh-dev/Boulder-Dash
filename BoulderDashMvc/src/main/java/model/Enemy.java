...
public class Enemy extends Sprite implements Ienemy {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -403212648061719573L;
	private Timer timer;
	...
	public void move() {
		...
		Point tablePoint[] = { new Point(getX(), getY() - Main.TILESIZE), new Point(getX() + Main.TILESIZE, getY()),
				new Point(getX(), getY() + Main.TILESIZE), new Point(getX() - Main.TILESIZE, getY()) };

		int[][] tableOrder = { { 1, 2, 3, 0 }, { 2, 3, 0, 1 }, { 3, 0, 1, 2 }, { 0, 1, 2, 3 } };
		...
		for (int i = 0; i < tableOrder[previous].length; i++) {
			...
			if (component != null) {
				if (component.getClass().getName().equals("javax.swing.JLayeredPane")
						|| component.getClass().getName().equals("Hero")) {
